...
export class CrudService<T extends ModelDefined<any, any>> {
	...
	async exec(promise: any, option: ICrudExecOption = { allowNull: true }) {
		try {
			...
			if ((result === undefined || result === null) && !option.allowNull)
				throw errorService.database.customError('Resource not found', 404)

			return result
		} catch (error) {
			console.log('ERROR ==> ', error)
			if (error instanceof BaseError)
				throw errorService.database.customError(
	...
	public async getList(
		...
		return await this.exec(
			this.modelWithScope(option.scope).findAndCountAll(option),
		)
	}
	async getItem(
		option: ICrudOption = { scope: ['defaultScope'], where: {} },
		allowNull = false,
	) {
		return await this.exec(this.modelWithScope(option.scope).findOne(option), {
			allowNull: allowNull,
		})
	}
	public async count(
	...
	async update(params: any, option?: ICrudOption) {
		const item = await this.exec(this.model.findByPk(option.where.id), {
			allowNull: false,
		})
